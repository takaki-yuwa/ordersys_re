...
public class OrderCompletedDAO {
	...
	// トッピング詳細を挿入するメソッド
	public boolean insertTopingDetails(Connection conn, String[][] topping_id, String[][] topping_quantity,
			String[] order_id) throws SQLException {
		...
		try (PreparedStatement pstmt = conn.prepareStatement(insertOrderDetailsSQL)) {
			for (int i = 0; i < order_id.length; i++) {
				for (int j = 0; j < topping_id[i].length; j++) {
					...
					pstmt.setInt(1, Integer.parseInt(topping_id[i][j])); // topping_id
					pstmt.setInt(2, Integer.parseInt(topping_quantity[i][j])); // topping_quantity
	...
	// 商品の在庫を更新するメソッド
	public void updateProductStock(Connection conn, String[] product_ids, String[] product_quantities)
			throws SQLException {
	...
	//トッピング在庫を更新するメソッド
	public void updateToppingStock(Connection conn, String[] topping_ids, String[] topping_quantities)
			throws SQLException {
